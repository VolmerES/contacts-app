...
$addresses = $conn->query("SELECT * FROM addresses WHERE user_id = {$_SESSION['user']['id']}");
		...
		<?php foreach ($addresses as $address) : ?>
            <div class="col-md-4 mb-3">
              <div class="card text-center">
                <div class="card-body">
                  <!-- Mostramos la dirección como título -->
                  <h3 class="card-title text-capitalize"><?= $address["address"] ?></h3>
                  <p class="m-2 text-muted">Contact ID asociado: <?= $address["contact_id"] ?></p>
